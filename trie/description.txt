...
then the output stream would look like this:

static const char __trie[] = {
  0x04,'A',0x80,0x00,0x19,0x00,0x06,'N',0x00,0x00,0x3A,0x00,0x00,'T',0x00,0x00,
  0x6A,0x00,0x00,'Z',0x80,0x00,0x82,0x00,0x06,'D','D','R','E','S','S',0x00,
  ...
  0x02,'E',0x00,0x00,0x4B,0x00,0x03,'M','E',0x00,0x10,'X','T','-',0x02,'A',
...
# ALGORITHMS FOR GENERATING THE TRIE STRING

A.  --- Algorithm for building internal tree ---
...
Loop:
 ...
 Cases:
   ...
   d) front of search-seg, front of node
      Example:	search-seg [KERS],1  node  [KES],2
                                            |
                                           [HOP],3
      New node  [KE]
                 |
...
# ---- Algorithm for traversing internal tree to insert byte positions ----
...
# ------ Free Internal Tree Algorithm ------

Set search stack level = 0

Loop:
  Point to list at current level
  if list not empty:
    remove first node
    if removed node has non-null down ptr:
      push down list on stack
      Loop
    free removed node
    Loop
    
  if level 0, then exit. We are done.
  pop stack
  free the popped list
  Loop
